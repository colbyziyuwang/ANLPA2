...
MODEL_SAVE_PATH = "models/sgd_poly_stock_doc2vec.pkl"
...
# ✅ Initialize pipeline (using polynomial kernel sketch)
model = make_pipeline(
    PolynomialCountSketch(degree=3, coef0=1, random_state=SEED),
...
for i in tqdm(range(num_batches), desc="Training batches"):
    ...
    # ✅ Fit Poly Kernel + SGD
    if is_first_batch:
        X_train_poly = model.named_steps['polynomialcountsketch'].fit_transform(X_train)
    else:
        X_train_poly = model.named_steps['polynomialcountsketch'].transform(X_train)
