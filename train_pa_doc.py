...
MODEL_SAVE_PATH = "models/pa_stock_doc2vec.pkl"
...
model = make_pipeline(
    PassiveAggressiveClassifier(loss='hinge', max_iter=1000, random_state=SEED, warm_start=True)
)
...
for i in tqdm(range(num_batches), desc="Training batches"):
    ...
    if is_first_batch:
        ...
        model.named_steps['passiveaggressiveclassifier'].set_params(class_weight={c: w for c, w in zip(classes, weights)})

    model.named_steps['passiveaggressiveclassifier'].partial_fit(
        X_train,
        y_train,
