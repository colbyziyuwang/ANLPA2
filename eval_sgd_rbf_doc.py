...
model_path = "models/sgd_rbf_stock_doc2vec.pkl"
conf_matrix_path = "images/confusion_matrix_sgd_rbf_doc2vec.png"
...
for test_file in tqdm(test_files, desc="Evaluating test files"):
    ...
    X_rbf = model.named_steps['rbfsampler'].transform(X)
    preds = model.named_steps['sgdclassifier'].predict(X_rbf)
...
plt.ylabel("Actual")
plt.title("Confusion Matrix - SGD + RBF + Doc2Vec")
plt.tight_layout()
