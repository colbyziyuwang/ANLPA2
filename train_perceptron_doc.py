...
MODEL_SAVE_PATH = "models/perceptron_stock_doc2vec.pkl"
...
model = Perceptron(max_iter=1000, random_state=SEED, warm_start=True)

# ✅ Training loop
num_batches = len(train_files) // batch_size + int(len(train_files) % batch_size > 0)
is_first_batch = True

for i in tqdm(range(num_batches), desc="Training Perceptron"):
    ...
    if is_first_batch:
        ...
        model.class_weight = {cls: w for cls, w in zip(classes, weights)}

    model.partial_fit(X_train, y_train, classes=classes if is_first_batch else None)
